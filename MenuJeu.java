...
public class MenuJeu
{
    ...
    public void checkTime(){
        ...
        if(hr!=current_hour){
            ...
            if(t.getToillete()<=10){
                t.setToillete(t.getToillete()+10);
            }else{
                t.setToillete(100);
            }
            if(t.getBonheur()>=2){
    ...
    Action moveToANewLocation = new AbstractAction() {
        public void actionPerformed(ActionEvent e) {
            ...
            if(t.getSommeil()>=5){
                ...
                JOptionPane.showMessageDialog(null, "Votre ami ne peut pas se deplacer, il est trop fatigué. Il se dirige vers sa chambre l'air abattu.");
                pageJ.remove(env);
                pageJ.remove(et);
            ...
            switch(env.getAction().getText()){
                case "Dormir":
                    if(t.getSommeil()<25){
                        t.dormir();
                        t.setDormir(1);
                        JOptionPane.showMessageDialog(null, "Votre ami dors, veuillez retourner plus tard.");
                        try{
                            dao.save(t);
                        }catch(IOException ioex){
                            ioex.printStackTrace();
                        ...
                        JOptionPane.showMessageDialog(null, "Votre ami a encore de l'energie, il ne souhaite pas dormir.");
                    }
                    break;
                case "Nourrir":
                    if(t.getSommeil()>5){
